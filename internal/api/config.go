...
func (s *Server) getConfig(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		// The sign-in providers to offer, in order, as {id, name}: id is the
		// route segment the button links to, name is what to call it. Empty
		// unless at least one provider is configured AND the redirect URI
		// resolved at startup (see SetSSO).
		"sso_providers":      s.ssoButtons(),
